...
?>
	...
	<?php 
				...
				if( 'no' != get_theme_mod('rockero_show_quote') ): 

				$rockeroQuoteTitle = '';
				$rockeroQuoteDesc = '';
				$rockeroQuoteContent = '';

				if( '' != get_theme_mod('rockero_quote_post') && 'select' != get_theme_mod('rockero_quote_post') ){

					$rockeroQuoteId = get_theme_mod('rockero_quote_post');

					if( ctype_alnum($rockeroQuoteId) ){

						$rockeroQuote = get_post( $rockeroQuoteId );

						$rockeroQuoteTitle = $rockeroQuote->post_title;
						$rockeroQuoteDesc = $rockeroQuote->post_excerpt;
						$rockeroQuoteContent = $rockeroQuote->post_content;
			...
			<div class="frontQuoteInnerContainer">
				...
				<?php 
					...
					if( '' != $rockeroQuoteDesc ){
						...
						echo esc_html($rockeroQuoteDesc);

					}else{

						echo esc_html(rockero_limitedstring($rockeroQuoteContent, 300));

					}

				?>			
				</p>
				<p><span><?php echo esc_html($rockeroQuoteTitle); ?></span></p>
	...
	<?php if( 'no' != get_theme_mod('rockero_show_social') ): ?>
	...
	<footer id="colophon" class="site-footer">
		...
		<div class="site-logo">
			...
			?>
			<p><?php esc_html_e( 'All rights reserved.', 'rockero' ); ?></p>
			...
			<?php if ( dynamic_sidebar('footer-left') ){ } else { ?>
				...
				<section class="widget">
					...
					<div class="widgetContainerInner">
						...
						<h2 class="widget-title"><span><?php _e( 'Pages', 'rockero' ); ?></span></h2>
                        <ul>
			...
			<?php if ( dynamic_sidebar('footer-middle') ){ } else { ?>
				...
				<section class="widget">
					...
					<div class="widgetContainerInner">
						...
						<h2 class="widget-title"><span><?php _e( 'Meta', 'rockero' ); ?></span></h2>
                        <ul>
                            <?php wp_register(); ?>
			...
			<?php if ( dynamic_sidebar('footer-right') ){ } else { ?>
				...
				<section class="widget">
					...
					<div class="widgetContainerInner">
						...
						<h2 class="widget-title"><span><?php _e( 'Archives', 'rockero' ); ?></span></h2>
                        <ul>
